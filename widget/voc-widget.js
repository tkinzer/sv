...
function getListReviews( clientID )
{
	...
	switch(envFLAG)
	{
		...
		/*
	--- USING HTTP POST
XMLHttpRequest cannot load http://staging.subscribervoice.com/get-client-reviews/. Response for preflight is invalid (redirect)
	--- USING HTTP GET
XMLHttpRequest cannot load http://staging.subscribervoice.com/get-client-reviews/?uniqueClientIdentifier=302&numberOfReviewsToReturn=5. 
Redirect from 'http://staging.subscribervoice.com/get-client-reviews/?uniqueClientIdentifier=302&numberOfReviewsToReturn=5' to 
'https://staging.subscribervoice.com/get-client-reviews/?uniqueClientIdentifier=302&numberOfReviewsToReturn=5' has been blocked by CORS policy: 
No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'http://timothykinzer:1234' is therefore not allowed access.
	--- USING HTTPS
jquery-3.1.1.min.js:4 GET https://staging.subscribervoice.com/get-client-reviews/?uniqueClientIdentifier=302&numberOfReviewsToReturn=5 net::ERR_INSECURE_RESPONSE
		*/
			envProtocol = 'https://staging.';
		break;
		case "testing":
			envProtocol = 'https://testing.';
		break;
		case "production":
			envProtocol = 'https://';
		break;
	...
	const apiURL = 
		envProtocol + 
		'subscribervoice.com/get-client-reviews/' +
		'?uniqueClientIdentifier=' + 
		clientID +
		'&numberOfReviewsToReturn=' + 
		numReviews;
	...
	var apiReviews = 
		$.ajax({
			...
			crossDomain : true,
			//data: {'uniqueClientIdentifier':clientID,'numberOfReviewsToReturn':'5','startIndex':'0'},
			type: envMethod,
		...
		.done(function(response){
			console.log("API Returned from get List of Reviews: ");
			console.log(response);
			
		})
		.fail(function(ev){
			console.log("Request to API failed on request or is missing a response");
			console.log(ev);
			
			fallbackMenu();
		});

	//END REQUEST TO SV API
}
...
function addSingleReview ( review )
{
	...
	reviewTextContainer.className = "review-text";
	if(review.comments.indexOf(" ") > 20 || review.comments.indexOf(" ") == -1)
	{
...
{
	console.log("Creating a menu to tell the user that there was an isssue or no reviews for the client at this time");
	
	var defaultHeader = document.createElement('div');

	var divScore = document.createElement('div');
	divScore.className = "text-center col-md-12 col-xs-12";
	var counter = 0;
	while( counter < 5)
	{
		var starbox = document.createElement('div');
	...
	document.getElementById('client-rating').appendChild(divScore);
	//stars score
	document.getElementById('client-rating-value').innerText = '-';
	//number reviews
	document.getElementById('client-reviews-total').innerText = '-';

	var defaultPrompt = document.createElement('div');
	defaultPrompt.className = "col-md-12 col-xs-12 default-prompt-text";
	defaultPrompt.innerHTML = "<div class='jumbotron'><h2 class='text-center col-md-12 col-xs-12' style='color:black;'>No Reviews at this Time<br/><small class='lead'>Try checking back later or be the first to leave a review.</small></h2></div>";
	
	document.getElementById('client-content').append(defaultPrompt);
}
